...
- 対象入口:
  - TEXT読込（`_load_text_file`）
  - WAV読込（`_load_wav_file`）
  - VMD保存（`_export_vmd`）
  - 履歴再読込（`_open_recent_text_file` / `_open_recent_wav_file`）
- 実装済みの最小防御:
  - 読込前のパス事前確認（空/非存在/ディレクトリ）
  - 保存前の最終パス確認（`.vmd` 補完後、保存先妥当性）
  - 履歴再読込前の危険値確認（空/非存在/ディレクトリ/拡張子不一致）
- 復帰方針:
  - 失敗時は `_show_warning(..., status=...)` で状態表示を失敗側へ統一
  - 読込/再読込失敗時は `current_timing_plan` や表示要素の不整合残留を抑制
  - 履歴再読込失敗時は該当履歴のみ除去し、メニューを更新
- 例外方針:
  - 想定外例外でも GUI 警告にフォールバックし、次操作で破綻しにくい状態へ復帰
