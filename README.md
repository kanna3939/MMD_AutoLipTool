...
現在のバージョン: `Ver 0.3.7.1`

## できること

- TEXT からひらがな列・母音列を生成
- WAV の基本情報読み込みと波形プレビュー表示
...
- TEXT: UTF-8 テキストファイル
- WAV: PCM WAV ファイル(MMDで正常に読めるWAVファイルに限る)
)
...
6. 必要であれば「開口維持」で口開けタイミングを維持し、「閉口スムーズ」で、口をゆるやかに閉じる調整を行えます
7. `VMD保存` から `.vmd` ファイルを書き出し7. 
...
サードパーティライブラリの概要と配布時の確認先は [THIRD_PARTY_LICENSES.md](THIRD_PARTY_LICENSES.md) を参照してください。  
