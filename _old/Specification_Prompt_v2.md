...
- 履歴項目からの再読込は既存 `_load_text_file` / `_load_wav_file` を再利用し、履歴専用の別読込ロジックは持たない
- 履歴再読込失敗時は、該当項目のみを除去し、対応する履歴メニューだけを即時更新する
- TEXT/WAV の履歴更新先・表示元・再読込導線は分離し、相互に混線しない
- 読込ダイアログの初期ディレクトリは、TEXT/WAV別の保持値から解決する
- 通常読込成功時と履歴再読込成功時に、対応する保持値を「選択ファイルの親ディレクトリ」で更新する
- 保持値が未設定/空/非存在/非ディレクトリの場合は、警告を追加せずフォールバック（空文字）で起動する
...
### 6.9 同名対応ファイルの自動補完読込（MS6B）

- 発動対象:
  - 通常読込（TEXT/WAV）
  - 履歴再読込（最近使ったTEXT/WAV）
- 非対象:
  - セッション外永続化の復元時発動（未対応のまま維持）
- 発動条件:
  - 主読込が成功した場合のみ補完判定を行う
  - 反対側の現在パスが既に設定済みの場合は補完しない
- 相方探索条件:
  - 同一フォルダ
  - 同一 stem
  - 拡張子のみ相補（`.txt` ↔ `.wav`）
...
- 挙動:
  - 主読込ごとに補完は1回だけ試行する
  - 補完側から逆方向補完は起こさない（再連鎖しない）
  - 相方未存在時は完全サイレントで終了する
  - 相方存在でも読込失敗時は自動補完経路で警告を出さず終了する
  - 自動補完専用の通知・確認ダイアログは追加しない
- 成功時反映:
  - 既存の `_load_text_file` / `_load_wav_file` 成功導線を再利用する
  - 履歴更新（TEXT/WAV別管理、重複先頭移動、上限10件、非混線）へ反映される
  - 読込開始ディレクトリ保持（`last_text_dialog_dir` / `last_wav_dialog_dir`）へ反映される
  - `current_timing_plan` の既存リセットと ready 状態更新の整合を維持する
