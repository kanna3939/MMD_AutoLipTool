...
- MS14-B5B: Whisper 解析可用性 hardening と処理中 UI 補強
...
| MS14-B5B | Whisper 解析可用性 hardening と処理中 UI 補強 | 解析キャンセル、timeout、遅延 callback 破棄、busy 中 close の安全化など、解析処理の操作可用性を補強する |
...
## 7.6 MS14-B5B Whisper 解析可用性 hardening と処理中 UI 補強

### 目的
wx 側の解析処理に対し、長時間処理への配慮、分析中止導線、timeout 時の warning 通知、遅延 callback 破棄などを追加し、ユーザーが安全に処理をコントロール・中止できる実用上の可用性を整える。

### 主対象
- `分析中止` ボタン導線
- soft timeout warning (150秒) と1回のみの警告
- 処理中ダイアログのフェーズ表示と warning 領域の分離・更新
- 中止済み job の結果が遅れて届いた際の破棄 (UI 反映保護)
- busy 中 close 確認と、close 時の安全なワーカー扱い
- UI 更新の main thread 移譲

### In Scope
- 解析処理ダイアログの拡張 (中止ボタン、フェーズ表示、warning)
- soft cancel 契約 (job 識別と破棄)
- 150秒の soft timeout (自動 abort ではなく warning のみ)
- busy 中 close における中止して終了導線
- frame/dialog が閉じた後の遅着 callback 遮断ガード

### Out of Scope
- Whisper backend の差し替え機能
- subprocess 強制終了などの hard cancel
- Whisper 内部の詳細 progress 提供
- timeout 設定値の settings 化 (B5B 時点では固定表示)
- B4 の基本解析パス自体の再設計

### 完了条件
1. 解析中に「中止」が行える
2. 150秒経過で warning が出るが自動終了はしない
3. timeout alert や warning と自然終了が競合しても状態が壊れない
4. 前段や中止した job の遅着 callback で UI が更新されたり破棄済みウィジェットに触れたりしない
5. これら異常系制御が B6 で統合確認できる状態になっている

---

## 7.7 MS14-B6 統合整理と parity closeout
...
- B4 は B2 / B3 に依存
- B5 は B2 / B3 / B4 に依存
- B5B は B4 と B5 の後に位置し、B4 / B5 の結果と UI 基盤に依存
- B6 は B1〜B5B 全体に依存
...
- B5B は B4 と B5 の導線上における長時間の待ち・中断といった異常系・操作境界を補強する
- B6 は全体を閉じる
...
6. **B5B** で解析キャンセル・timeout などの可用性を補強する
7. **B6** で統合し、依存・文書・配布整合を閉じる
...
### 10.6 MS14-B5B への流入禁止
- Whisper 内部処理の差し替え
- subprocess kill のような hard cancel
- 再実行救済（完全ハング時等の）
- B4 の基本解析パイプライン再設計
...
## 11.6 B5B
- soft cancel での中止状態と結果破棄
- 150 sec での timeout warning (1回のみ)
- busy 中 close における中止して終了
- 遅着 callback の UI 反映ブロック
