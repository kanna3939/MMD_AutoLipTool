...
* `src/gui/i18n_strings.py`
  ...
  * 日本語 / EN の文字列定義の正本
  * キーによる UI 文字列取得
  * MS9 では全面導入は必須ではないが、文字列管理の受け皿候補として扱う
...
- `i18n_strings.py` / `settings_store.py` / `layout_helpers.py` / `frame_utils.py` は、本書では予定ファイルとして扱う。
- MS9 では上記ファイルの全面導入は必須ではないが、テーマ切替・左右分割比率・文字列管理の受け皿の置き場整理は対象に含む。
- 上記予定ファイルは、2026-03-22 時点ではまだリポジトリに実体がない。
...
- 次セッションでは、MS9 本体や shared viewport 正本責務を再設計せず、主に見た目・余白・最終 UX の微調整を `MS9-2` として扱う。
- 想定対象:
  - 右表示領域共通スクロールバーの見た目調整
  - テーマとの馴染ませ
  - splitter リサイズ時の違和感低減
  - 実機確認結果に基づく軽微な体感修正
